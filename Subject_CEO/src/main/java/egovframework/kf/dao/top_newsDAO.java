 ...
 */
@Repository
public class top_newsDAO {
	private static final Logger logger = LoggerFactory.getLogger(top_newsDAO.class);
	...
	public RestResultVO top_newsSearch(ParameterVO paramVO) throws Exception {
		...
		if (!paramVO.getStartDate().isEmpty() && !paramVO.getEndDate().isEmpty()){
			query = dcUtil.makeRangeQuery("updated_date", paramVO.getStartDate().replace(".", "")+"000000", paramVO.getEndDate().replace(".", "")+"999999", query) ;
		}
		...
		searchVO.setFields(konanPropertiesService.getString("top_newsField"));
		searchVO.setFrom(konanPropertiesService.getString("top_newsFrom"));
		searchVO.setHilightTxt(konanPropertiesService.getString("top_newsHilight"));
		...
		logger.debug("RESTURL top_news : " + postParamData);
