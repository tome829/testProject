...
<c:if test="${top_newsTotal > 0}">
<section>
	<h3>테스트 1번
	<c:if test="${params.category != 'TOTAL'}">	
		<small><fmt:formatNumber value="${top_newsTotal}" groupingUsed="true"/>건</small>
	</c:if>

	</h3>
	<ul class="media-list">
	<c:forEach var="result" items="${top_newsList}" varStatus="status">
	<fmt:parseDate value="${result.created_time}" var="dateFmt" pattern="yyyyMMddHHmmss"/>
	<fmt:parseDate value="${result.updated_date}" var="updateFmt" pattern="yyyyMMddHHmmss"/>
    <li class="media">
